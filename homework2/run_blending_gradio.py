...
def create_mask_from_points(points, img_h, img_w):
    ...
    返回:
        np.ndarray: 形状为 (img_h, img_w) 的二进制掩码。
    """
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    """
    计算前景图像和混合图像在掩码区域内的拉普拉斯损失。

    参数:
        foreground_img (torch.Tensor): 前景图像张量。
        foreground_mask (torch.Tensor): 前景掩码张量。
        blended_img (torch.Tensor): 混合图像张量。
        background_mask (torch.Tensor): 背景掩码张量。

    返回:
        torch.Tensor: 计算出的拉普拉斯损失。
    """
    # 定义Laplacian核并扩展至匹配输入通道数
    laplacian_kernel = torch.tensor([[-1, -1, -1],
                                     [-1, 8, -1],
                                     [-1, -1, -1]], dtype=torch.float32, device=foreground_img.device).unsqueeze(
        0).unsqueeze(0)
    channels = foreground_img.shape[1]
    laplacian_kernel = laplacian_kernel.expand(channels, 1, 3, 3)

    # 计算前景图像和融合图像的Laplacian
    ...
    # 确保背景蒙版的维度正确
    if background_mask.dim() == 3:  # 如果是3D张量 (N, H, W)
        mask = background_mask.unsqueeze(1).expand(-1, channels, -1, -1).float()  # 扩展为 (N, C, H, W)
    else:  # 否则假设已经是4D张量 (N, C, H, W)
        mask = background_mask.float()

    # 计算蒙版区域内的损失
    loss = torch.mean((foreground_lap - blended_lap) ** 2 * mask)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    """
    使用泊松融合将前景多边形区域融合到背景图像中。

    参数:
        foreground_image_original (PIL.Image): 原始前景图像。
        background_image_original (PIL.Image): 原始背景图像。
        dx (int): 水平偏移量。
        dy (int): 垂直偏移量。
        polygon_state (dict): 当前多边形的状态。

    返回:
        np.ndarray: 混合后的图像，类型为 numpy 数组。
    """
